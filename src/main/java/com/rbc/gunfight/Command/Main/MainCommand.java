package com.rbc.gunfight.Command.Main;

import com.rbc.gunfight.Command.CommandToDo;
import com.rbc.gunfight.Command.Main.Info.Info;
import com.rbc.gunfight.Command.Main.Reload.Reload;
...
public class MainCommand extends CommandToDo {
    private final ArrayList<CommandToDo> baseCMDS;

    public MainCommand(CommandToDo superCmd, ModeManager modeManager) {
        super(superCmd, modeManager);
        name = "main";
        baseCMDS = new ArrayList<>();
        baseCMDS.add(new Info(this, modeManager));
        baseCMDS.add(new Reload(this, modeManager));
    }
    ...
    public void reloadSubCmds(Player player) {
        ...
        if (map != null) {
            subCmds.addAll(map.getModeMain().getMainCmds());
        }
    }
        ...
        if (sender.hasPermission("gunfight.main")) {
            ...
            sender.sendMessage(
                    Config.langWithPrefix("no_permission")
                            .replace("{permission}", "gunfight.main"));
        }
    }
