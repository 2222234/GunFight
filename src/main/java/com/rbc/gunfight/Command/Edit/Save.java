...
public class Save extends CommandToDo {
    private final MapManager mapManager;

    public Save(CommandToDo superCmd, ModeManager modeManager) {
    ...
    public void toDo(CommandSender sender, Command command, String label, String[] args) {
        if (args.length == 0) {
            ...
            if (map != null) {
                if (map.save()) {
                    sender.sendMessage(
                            Config.langWithPrefix("save_success")
                                    .replace("{map}", map.getName()));
                } else {
                    sender.sendMessage(
                            Config.langWithPrefix("save_fail")
                                    .replace("{map}", map.getName()));
                }
            } else {
                sender.sendMessage(
